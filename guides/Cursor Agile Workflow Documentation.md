...
If I am starting a brand new project (with or without an existing code template) I have a few options:
...
Once you feel it's in a good spot - you can mark the file as status: approved.

At this point, I would start another chat and with the workflow - the agent will first check for the prd, and then if it's approved, will offer to create (if not already existing and approved) the architecture file - and similar a new chat window with the workflow will search for the new first or in-progress story.
...
2. **Testing**
   - Have the AI Write tests before implementation - a fun exercise in TDD
...
4. **Context Management**
   ...
   - Consider making a leaner workflow when you are in story execution mode - that does not need all of the templates and overhead of how to create a prd and an architecture. But you will need to consider what other files or parts of other files it might need reference to retain the plot. This is why currently I still use the full workflow.
...
---
[[_NoteCompanion/Backups/Cursor Agile Workflow Documentation_backup_20250410_195522.md | Link to original file]]
#agentic_workflow